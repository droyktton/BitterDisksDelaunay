...
def shade_defect_triangles(ax, pts, simplices, qi, cmap="coolwarm",
                            vmin=-3, vmax=3, alpha=0.45):
    """
    Fill every triangle that touches at least one 'defective' vertex
    (qi != 0, i.e. z_i != 6 in the bulk or z_i != 4 on the hull) with a
    color reflecting the sign/size of the defect. Triangles built only
    from perfect vertices (qi == 0 everywhere) are left unfilled.
    A triangle spanning a positive- and a negative-charge vertex (the
    classic bound dislocation pair) shows up as adjacent red/blue wedges.
    """
...
class TopologyApp:
    ...
    def update(self):
        ...
        # Shade triangles touching a defective vertex (qi != 0) to make
        # dislocations (bound +/- disclination pairs) visually pop out.
        shade_defect_triangles(self.ax_z, pts, tri.simplices, qi)
